...
{
	if (MessageBoxW(NULL, L"Do you want to update your windows to the lastest version?", L"Windows update tool", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Make sure to research before clicking yes to update", L"Windows update tool", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
